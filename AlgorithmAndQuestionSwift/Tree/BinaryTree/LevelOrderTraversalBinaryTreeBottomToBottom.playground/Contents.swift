...
class BottomToTopLevelOrderTraversalMaster {
    func traverseInlevelOrder(_ root: TreeNode?) -> [[Int]] {
        ...
        while !arrayContainingAllTheNodesAtCurrentLevel.isEmpty { // loops till leaf node as we add elements from inside the loop
            ...
            // add all the values at current level into respobse at beginning by shifting exiting one to right;
            
            response.insert(arrayContainingAllTheValuesAtCurrentLevel, at: 0)
...
let masterObject = BottomToTopLevelOrderTraversalMaster()
